...
export const TeamLeaderboard: React.FC<TeamLeaderboardProps> = ({ 
  ...
  // In team mode, only include those who are primary team players (bắn chính) AND belong to a real team
  const activeAthletes = useMemo(() => {
    return athletes.filter((a) => a.isPrimaryTeam && !isNoTeam(a.team));
  }, [athletes]);
  ...
  const activeAthletesForCalculation = useMemo(() => {
    if (competitionMode === "team") {
      // In team mode, only include primary team players (bắn chính) with real teams
      return athletes.filter((a) => a.isPrimaryTeam && !isNoTeam(a.team));
    } else {
      // In individual mode:
      if (activeTab === "survival") {
        // "Team Bắn Chính" -> only include primary team players (bắn chính) with real teams
        return athletes.filter((a) => a.isPrimaryTeam && !isNoTeam(a.team));
      } else {
        // "500 Anh Em" -> include all athletes belonging to real teams
        return athletes.filter((a) => !isNoTeam(a.team));
      }
    }
    ...
    for (let r = 0; r < distances.length; r++) {
      ...
      currentRoundTeams.forEach((teamName: string) => {
        ...
        activeMembers.forEach((memb) => {
          const hits = memb.scores[dist.id] || [];
          roundPoints += getPointsSum(hits, isDirectMode);
        ...
        if (isDirectMode && teamDirectMaxPoints !== undefined && teamDirectMaxPoints > 0) {
          ...
          if (totalPossShots === 0) {
            const totalShotsCountInRounds = isCum ? (r + 1) * effectiveShotsCount : effectiveShotsCount;
  ...
  const activeTeamScores = useMemo(() => {
    ...
    if (hasMaxRoundScoreConf) {
      ...
      teamsList.forEach((teamName) => {
        const members = activeAthletes.filter((a) => {
          const raw = a.team.trim();
          return !isNoTeam(raw) && raw === teamName && a.isPrimaryTeam && a.status !== "Bỏ thi";
        });
                        ...
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <span className="text-[9px] font-mono text-gray-400 block">
                              VSC: {getCleanVscNumber(member.vscNumber, member.id)} {member.bibNumber && `| BIB: ${getCleanBibNumber(member.bibNumber, member.id)}`}
                            </span>
                          </div>
                        </div>
