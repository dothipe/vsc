...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const roundResultsIndividual = useMemo(() => {
    return calculateRounds(activeAthletesIndividual, actualIndDistances, effectiveShotsCountIndividual, effectiveDirectMaxPointsIndividual, directMaxShots);
  }, [activeAthletesIndividual, actualIndDistances, effectiveShotsCountIndividual, effectiveDirectMaxPointsIndividual, directMaxShots]);
  ...
  const roundResultsTeam = useMemo(() => {
    return calculateRounds(activeAthletesTeam, actualTeamDistances, effectiveShotsCountTeam, effectiveDirectMaxPointsTeam, teamDirectMaxShots);
  }, [activeAthletesTeam, actualTeamDistances, effectiveShotsCountTeam, effectiveDirectMaxPointsTeam, teamDirectMaxShots]);
    ...
    for (let r = 0; r < actualTeamDistances.length; r++) {
      ...
      currentRoundTeams.forEach((teamName: string) => {
        ...
        if (teamIsDirectMode && teamDirectMaxPoints !== undefined && teamDirectMaxPoints > 0) {
          ...
          const totalPossShots = activeMembers.length * (isCum ? (r + 1) * effectiveShotsCountTeam : effectiveShotsCountTeam);
                  ...
                  <label className={`flex items-center justify-between p-3 border-2 rounded-2xl cursor-pointer transition-all hover:bg-slate-50 dark:hover:bg-slate-850 ${
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-gray-400 block font-normal font-sans">Bảng xếp hạng đồng đội - BXH ({paginatedTeamsTeamVisible.length} ảnh)</span>
                      </div>
                    </div>
                  </label>
                </div>
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-2">
                          {renderDists.map((dist) => {
                            ...
                            const hitCount = getHitCount(hits);
                            const score = hitCount * dist.multiplier;
                            const actualShots = dist.shotsCount !== undefined && dist.shotsCount !== null && dist.shotsCount > 0
                              ? dist.shotsCount
                              : (competitionMode === "team" ? (teamShotsCount || shotsCount) : shotsCount);
                            const isDistDirectMode = actualShots === 1;
                              ...
                              >
                                ...
                                <div className="flex flex-col shrink-0 text-left">
                                  ...
                                  <span className="text-[9px] text-indigo-600 font-extrabold mt-0.5">
                                    {isDistDirectMode ? `Điểm số ${hitCount}/${actualShots}` : `Trúng ${hitCount}/${actualShots}`}
                                  </span>
                                </div>
                                ...
                                <div className="flex-1 flex justify-center overflow-x-auto py-0.5 scrollbar-thin">
                                  <div className="flex gap-1 justify-center items-center">
                                    {Array(actualShots).fill(null).map((_, shotIdx) => {
