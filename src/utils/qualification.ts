...
import { ScoreAggregationLayer } from "../foundation/scoreAggregationLayer";
import { getSoloRoundsFromDist } from "../engines/rankingEngine";
import { isAthleteEliminatedInPrevStage } from "./athleteUtils";

export function compareSoloRounds(roundsA: (number | null | undefined)[] = [], roundsB: (number | null | undefined)[] = []): number {
  const maxLen = Math.max(roundsA.length, roundsB.length);
  for (let i = 0; i < maxLen; i++) {
    const aVal = typeof roundsA[i] === "number" ? (roundsA[i] as number) : 0;
    const bVal = typeof roundsB[i] === "number" ? (roundsB[i] as number) : 0;
    if (aVal !== bVal) {
      return aVal - bVal;
    }
  }
...
export function computeSoloFraction(soloRoundsArr?: (number | null | undefined)[], fallbackHits?: number): number {
  if (Array.isArray(soloRoundsArr) && soloRoundsArr.length > 0) {
    let fraction = 0;
    for (let k = 0; k < soloRoundsArr.length; k++) {
      const hits = typeof soloRoundsArr[k] === "number" ? (soloRoundsArr[k] as number) : 0;
      fraction += hits / (1000 * Math.pow(10, k));
    }
    return fraction;
  }
  if (typeof fallbackHits === "number" && fallbackHits > 0) {
    return fallbackHits / 1000;
  }
  return 0;
}
...
export function calculateRounds(
  ...
  for (let r = 0; r < distances.length; r++) {
    ...
    // We also strictly exclude any athlete who was explicitly eliminated in previous rounds.
    const currentRoundParticipants = athletes
      .filter(ath => {
        if (ath.status === "Bỏ thi") return false;
        if (isAthleteEliminatedInPrevStage(ath, dist.id, distances)) return false;
        return activeIds.includes(ath.id) || (ath.scores[dist.id] && ath.scores[dist.id].length > 0 && ath.scores[dist.id].some(v => v !== null && v !== undefined));
      })
      .map(ath => ath.id);
    ...
    athletes.forEach(ath => {
      ...
      const normPkg = ScoreAggregationLayer.normalize({ ...ath, scores: { [dist.id]: hits } } as Athlete, [dist]);
      const distScoreObj = normPkg.distances[dist.id];
      const roundHits = distScoreObj ? distScoreObj.totalHits : 0;
      const roundScore = distScoreObj ? distScoreObj.weightedPoints : 0;
      ...
      // Hidden/comparison score that includes solo shootout hits using lexicographical weighting
      const soloRoundsArr = getSoloRoundsFromDist(ath, dist);
      const soloHitsVal = soloRoundsArr.length > 0 ? soloRoundsArr.reduce((a, b) => a + (b || 0), 0) : ath.soloHits?.[dist.id];

      const soloFraction = computeSoloFraction(soloRoundsArr, soloHitsVal);
      const displayScoreWithSolo = displayScore + soloFraction;
      ...
      if (directMaxPoints !== undefined && directMaxPoints > 0) {
        ...
        const basePerRoundShots = (directMaxShots !== undefined && directMaxShots !== null && directMaxShots > 0) ? directMaxShots : shotsCount;
        ...
        if (isCum) {
          for (let i = 0; i <= r; i++) {
            ...
            const wasShot = ath.scores[d.id] && ath.scores[d.id].length > 0 && ath.scores[d.id].some(v => v !== null && v !== undefined);
            if (wasShot) {
              totalPossShots += basePerRoundShots;
            }
          }
          if (totalPossShots === 0) {
            totalPossShots = basePerRoundShots;
          ...
          if (slotsLeft > 0 && slotsLeft < contenders.length) {
            ...
            contenders.forEach(id => {
              ...
              if (isUnshotInThisRound) {
                ...
                const soloRounds = getSoloRoundsFromDist(ath, dist);
                const hasSolo = soloRounds.length > 0 || (ath?.soloHits?.[dist.id] !== undefined && ath.soloHits[dist.id] !== null);
                if (!hasSolo) {
            ...
            if (unshotContenders.length > 0 || pendingSoloContenders.length > 0) {
              ...
              // Shootout is completed! Work out winner using lexicographical multi-round solo evaluation
              const contendersWithSolo = contenders.map(id => {
                const ath = athleteMap.get(id);
                const soloRounds = getSoloRoundsFromDist(ath, dist);
                return { id, soloRounds };
              });

              // Sort descending by soloRounds (lexicographically, Solo #1 first, then Re-Solo #2, etc.)
              contendersWithSolo.sort((a, b) => compareSoloRounds(b.soloRounds, a.soloRounds));

              const winnerObj = contendersWithSolo[slotsLeft - 1];
              const loserObj = contendersWithSolo[slotsLeft];

              const isTieAtBoundary = compareSoloRounds(winnerObj.soloRounds, loserObj.soloRounds) === 0;

              if (isTieAtBoundary) {
                // We have a tie at the solo cutoff boundary! Resolo is required for anyone tied with the boundary winner.
                const resoloCandidates = contendersWithSolo
                  .filter(c => compareSoloRounds(c.soloRounds, winnerObj.soloRounds) === 0)
                  .map(c => c.id);
                roundResoloIds = resoloCandidates;

                const surelySoloPassed = contendersWithSolo
                  .filter(c => compareSoloRounds(c.soloRounds, winnerObj.soloRounds) > 0)
                  .map(c => c.id);
                const surelySoloFailed = contendersWithSolo
                  .filter(c => compareSoloRounds(c.soloRounds, winnerObj.soloRounds) < 0)
                  .map(c => c.id);
